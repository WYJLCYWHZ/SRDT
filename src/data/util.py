...
def read_img(env, path, size=None):
    ...
    if env is None:  # img
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)#read
    else:
        img = _read_img_lmdb(env, path, size)
    img = img.astype(np.float32) / 255.#resize
    if img.ndim == 2:
...
def read_mat(env, path, size=None):
    ...
    if env is None:  # img
        ...
        data_V = scipy.io.loadmat(path)
        img = np.float32(data_V[str('AA')])#rename the variable
    else:
        img = _read_img_lmdb(env, path, size)
