...
class DenoisingModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            optim_params.append(self.log_sigma_sq)
    ...
    def feed_data(self, state, LQ, GT=None, task_id=None):
        ...
        if GT is not None:
            ...
            self.state_0 = None
        self.task_id = task_id  # 保存任务号列表

    def optimize_parameters(self, step, timesteps, sde=None, task_id=None):
        ...
        # 假设 self.loss_fn 返回每个任务的损失列表
        # modified
        task_losses = self.loss_fn(xt_1_expection, xt_1_optimum)  # [loss1, loss2, loss3]
        # print(f"[DEBUG] task_id: {task_id}, len: {len(task_id)}")
        # print(f"[DEBUG] task_losses: {task_losses}, len: {len(task_losses)}")
        # print(f"[DEBUG] task_losses: {task_losses}, shape: {task_losses.shape}")


        total_loss = 0
        for task_loss, tid in zip(task_losses, task_id):

            sigma_sq = torch.exp(self.log_sigma_sq[tid])  # 还原 σ_t^2
            weighted_loss = (0.5 / sigma_sq) * task_loss + 0.5 * self.log_sigma_sq[tid]

            # print("tid:{}".format(tid))
            # print("weighted_loss:{}".format(weighted_loss))
        ...
        self.optimizer.step()
        self.ema.update()
        self.log_dict["loss"] = total_loss.item()
