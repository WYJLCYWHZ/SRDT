#latent-seismic-restoring.py continues training the UNet pretrained in unet-latent.py for the seismic data restoration task, which corresponds to the second stage described in the paper.
...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # print(f"Current step: {current_step}, LQ Batch size: {LQ.shape[0]}")
            task_names = train_data["task_name"]
            task_ids = [TASK_MAP.get(name, 0) for name in task_names]
            # print(f"Task IDs: {task_ids}")

            latent_LQ, _ = model.encode(LQ.to(device))
            latent_GT, _ = model.encode(GT.to(device))
            # print(f"latent_LQ Batch size: {latent_LQ.shape[0]}")
            ...
            model.feed_data(states, latent_LQ, latent_GT, task_id=task_ids)# modified
            model.optimize_parameters(current_step, timesteps, sde, task_id=task_ids) # modified           
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    # calculate PSNR
                    print(f"util.calculate_psnr(output, gt_img): {util.calculate_psnr(output, gt_img)}")
